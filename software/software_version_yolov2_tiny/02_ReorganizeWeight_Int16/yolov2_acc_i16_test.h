...
void generate_iofm_offset(int32_t* in_ptr[32], int32_t* out_ptr[32], int32_t *Memory_buf, network *net)
{
#define ROUTE16_LEN (26*26*512/2)
#define CONV27_LEN (13*14*256/2)
#define CONV24_LEN (13*14*1024/2)

	int32_t *Memory_top = Memory_buf+512;
	int32_t *Memory_bottom = Memory_top + MEM_LEN;
	int x;
	for(x=0;x<16;x++)
	{
		if(x%2==0)
...
{
	int weight_offset[16] = {432, 4608, 18432, 73728, 294912, 1179648, 4718592, 4718592, 217600,0,0,0,0,0,0,0};
	...
	int32_t *Weight_buf = (int32_t *)malloc(file_size);
	if(!Weight_buf) printf("Weight_buf alloc failed.\n");
	printf("weight's size = %d\n", file_size);
	FILE *fp_w = fopen("weight_reorg_ap16.bin", "rb");
    	if(!fp_w) file_error("weight_reorg_ap16.bin");
	fread(Weight_buf, sizeof(int32_t), file_size/4, fp_w);
	fclose(fp_w);
	...
	int32_t *Beta_buf = (int32_t *)malloc(file_size);
	if(!Beta_buf) printf("Beta buf alloc failed.\n");
	printf("bias's size = %d\n", file_size);
	FILE *fp_b = fopen("bias_ap16.bin", "rb");
    	if(!fp_b) file_error("bias_ap16.bin");
	fread(Beta_buf, sizeof(int32_t), file_size/4, fp_b);
	fclose(fp_b);
		...
		}else
		{
			switch(l.type)
			{
				...
				//		memcpy((int16_t *)(region_buf) + k*26, (int16_t *)(tmp_ptr_int32) + k*32, 26*sizeof(int16_t));
					reorg_cpu((int16_t *)region_buf, ofm_w, ofm_h, 4, 2, (int16_t *)region_buf2);
					for(int k = 0; k<13*256; k++)
						memcpy((int16_t *)(region_buf) + k*14, (int16_t *)(region_buf2) + k*13, 13*sizeof(int16_t));
				...
				case REGION:
					...
					tmp_ptr_int16 = (int16_t *)(in_ptr[i]);
					tmp_iofmQ_pow = pow(2.0, -iofmQ[offset_index]);
					for(int k = 0; k<13*425; k++)
						for(int j = 0; j < 13; j++)
						{
							region_buf[k*13 + j] = tmp_ptr_int16[k*14 + j]*tmp_iofmQ_pow;
						}
